...
var debug = require('debug')('feathers-knex');
  ...
  _createClass(Service, [{
    ...
    value: function db() {
      ...
      if (params.transaction) {
        var _params$transaction = params.transaction,
            trx = _params$transaction.trx,
            id = _params$transaction.id;
    ...
    key: 'init',
    value: function init(opts, cb) {
      var k = this.knex;
      var table = this.table;

      return k.schema.hasTable(table).then(function (exists) {
        if (!exists) {
          debug('creating ' + table);
          return k.schema.createTable(table, cb).then(function (res) {
            return res;
          });
        } else {
    ...
    value: function createQuery() {
      ...
      var _filter = (0, _feathersQueryFilters2.default)(params.query || {}),
          filters = _filter.filters,
          query = _filter.query;

      var q = this.db(params).select([this.table + '.*']);

      // $select uses a specific find syntax, so it has to come first.
      if (filters.$select) {
        var _db;

        q = (_db = this.db(params)).select.apply(_db, _toConsumableArray(filters.$select.concat(this.table + '.' + this.id)));
      }
    ...
    value: function _find(params, count) {
      ...
      if (count) {
        var countQuery = this.db(params).count(this.id + ' as total');
    ...
    value: function _create(data, params) {
      ...
      return this.db(params).insert(data, this.id).then(function (rows) {
        var id = typeof data[_this3.id] !== 'undefined' ? data[_this3.id] : rows[0];
        return _this3._get(id, params);
      }).catch(_errorHandler2.default);
    }
  }, {
    key: 'create',
    value: function create(data, params) {
      ...
      if (Array.isArray(data)) {
        return Promise.all(data.map(function (current) {
          return _this4._create(current, params);
        }));
      }
    ...
    value: function update(id, data, params) {
      ...
      return this._get(id, params).then(function (oldData) {
        ...
        delete newObject[_this6.id];

        return _this6.db(params).where(_this6.id, id).update(newObject).then(function () {
          // NOTE (EK): Restore the id field so we can return it to the client
          newObject[_this6.id] = id;
          return newObject;
        });
      }).catch(_errorHandler2.default);
    }
    ...
    value: function remove(id, params) {
      ...
      return this._find(params).then(function (page) {
        var items = page.data;
        var query = _this7.db(params);

        _this7.knexify(query, params.query);
